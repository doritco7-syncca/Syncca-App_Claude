// PersonalCard.jsx — Syncca · Multi-Language Edition
// Props:
...
// ─── Field Definitions (language-aware) ──────────────────────────────────────
// IMPORTANT: Select option VALUES stay Hebrew — FIELD_MAPS uses Hebrew as lookup keys.
// Only the displayed LABELS are translated. Airtable storage is unaffected.
function getFields(chatLang) {
  const maritalOptions = [
    { value: "רווק/ה",            label: uiStr(chatLang, "רווק/ה",            "Single",            "Ledig") },
    { value: "זוגיות",            label: uiStr(chatLang, "זוגיות",            "In a relationship", "In einer Beziehung") },
    { value: "נשוי/ה",            label: uiStr(chatLang, "נשוי/ה",            "Married",           "Verheiratet") },
    { value: "גרוש/ה",            label: uiStr(chatLang, "גרוש/ה",            "Divorced",          "Geschieden") },
    { value: "אלמן/ה",            label: uiStr(chatLang, "אלמן/ה",            "Widowed",           "Verwitwet") },
  ];
  const genderOptions = [
    { value: "אישה",              label: uiStr(chatLang, "אישה",              "Woman",             "Frau") },
    { value: "גבר",               label: uiStr(chatLang, "גבר",               "Man",               "Mann") },
    { value: "נון-בינארי/ת",     label: uiStr(chatLang, "נון-בינארי/ת",      "Non-binary",        "Nicht-binär") },
    { value: "מעדיף/ה לא לציין", label: uiStr(chatLang, "מעדיף/ה לא לציין",  "Prefer not to say", "Keine Angabe") },
  ];
  // Age range numbers are universal — no translation needed, mapped to uniform objects
  const ageOptions = ["20-29","30-39","40-49","50-59","60-69","70-79","80-100"]
    .map(o => ({ value: o, label: o }));

  return [
    {
      key: "First_Name",
      label: uiStr(chatLang, "שם פרטי",     "First Name",         "Vorname"),
      type: "text",
      placeholder: uiStr(chatLang, "שמך",   "Your name",          "Dein Name"),
    },
    {
      key: "Full_Name",
      label: uiStr(chatLang, "שם מלא",       "Full Name",          "Vollständiger Name"),
      type: "text",
      placeholder: uiStr(chatLang, "שם מלא", "Full name",          "Vollständiger Name"),
    },
    {
      key: "Age_Range",
      label: uiStr(chatLang, "טווח גיל",     "Age Range",          "Altersbereich"),
      type: "select",
      options: ageOptions,
    },
    {
      key: "Marital_Status",
      label: uiStr(chatLang, "מצב משפחתי",   "Marital Status",     "Familienstand"),
      type: "select",
      options: maritalOptions,
    },
    {
      key: "Gender",
      label: uiStr(chatLang, "מגדר",         "Gender",             "Geschlecht"),
      type: "select",
      options: genderOptions,
    },
    {
      key: "Language_Preference",
      label: uiStr(chatLang, "שפה מועדפת",   "Preferred Language", "Bevorzugte Sprache"),
      type: "text",
      placeholder: uiStr(chatLang, "עברית / English", "Hebrew / English / German", "Hebräisch / English / Deutsch"),
    },
  ];
}
...
export default function PersonalCard({
  ...
  // Reverse-map: Airtable stores English values, UI stores Hebrew internally
  ...
  function resolveExplanation(c) {
    ...
    if (chatLang === "de") return entry?.explanationDE || entry?.explanationEN || entry?.explanation || "";
    return entry?.explanation || c.explanation
      || uiStr(chatLang, "מושג מרכזי בשפה של זוגיות נקייה.", "A key concept in the language of clean relationship.", "Ein Schlüsselbegriff der Sprache der reinen Beziehung.");
  }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSaveState("error");
      setErrMsg(e.message || uiStr(chatLang, "שגיאה בשמירה", "Save error", "Speicherfehler"));
    }
  ...
  const saveLabel = {
    idle:   uiStr(chatLang, "שמירה  💾",                   "Save  💾",               "Speichern  💾"),
    saving: uiStr(chatLang, "שומר...",                      "Saving...",              "Speichert..."),
    saved:  uiStr(chatLang, "✓  נשמר!",                    "✓  Saved!",              "✓  Gespeichert!"),
    error:  uiStr(chatLang, "שגיאה — כדאי לנסות שוב",     "Error — please try again","Fehler — bitte erneut"),
  }[saveState];
  ...
  const FIELDS   = getFields(chatLang);
        ...
        <div className="pc-slab" style={{
          ...
          <div style={{
            ...
            <button className="close-btn" onClick={onClose}
              title={uiStr(chatLang, "חזרה לשיחה", "Back to chat", "Zurück zum Chat")}>✕</button>

            <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
              {/* Syncca logo SVG — unchanged */}
            ...
            <div style={{
              ...
              <div>
                <div style={{
                  ...
                  {form.Full_Name || form.First_Name
                    || uiStr(chatLang, "שם מלא", "Full Name", "Vollständiger Name")}
                </div>
                <div style={{
                  fontFamily: "'Alef', sans-serif",
                  fontSize: "1.03rem", color: COLORS.muted,
                  direction: uiDir(chatLang),
                }}>{record.email || ""}</div>
              </div>
            </div>
              ...
              {FIELDS.map(f => (
                <div key={f.key}>
                  ...
                  {f.type === "select" ? (
                    ...
                    >
                      <option value="">
                        {uiStr(chatLang, "בחרי...", "Select...", "Auswählen...")}
                      </option>
                      {f.options.map(o => (
          ...
          <div style={{ flexShrink: 0, position: "relative", margin: "0 18px" }}>
            ...
            <span style={{
              ...
              {uiStr(chatLang, "✦ המושגים שלי", "✦ My Concepts", "✦ Meine Konzepte")}
            </span>
          </div>

          {/* ── BOTTOM PANEL — saved concepts ─────────────────────────────── */}
          <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 12px", minHeight: 0 }}>
            {savedConcepts.length === 0 ? (
              <div style={{
                color: COLORS.muted, fontSize: "0.98rem",
                fontFamily: "'Alef', sans-serif",
                direction: uiDir(chatLang), lineHeight: 1.6,
                textAlign: uiAlign(chatLang),
              }}>
                {uiStr(
                  chatLang,
                  "לחיצה על מושג מודגש בשיחה תשמור אותו כאן.",
                  "Tapping a highlighted concept in the chat saves it here.",
                  "Ein Antippen eines hervorgehobenen Konzepts speichert es hier."
                )}
              </div>
            ) : (
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", direction: uiDir(chatLang) }}>
                {savedConcepts.map((c, i) => (
                  <div key={i} style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (activeConcept?._idx === i) setActiveConcept(null);
                        onDeleteConcept?.(c);
                      }}
            ...
            {activeConcept && (
              <div style={{
                ...
                <div style={{
                  ...
                  color: getConceptColors(findEntry(activeConcept) || activeConcept).text,
                  marginBottom: "6px",
                  textAlign: uiAlign(chatLang),
                }}>
                  {resolveWord(activeConcept)}
                </div>
                <div style={{
                  fontFamily: "'Alef', sans-serif",
                ...
                <button onClick={() => setActiveConcept(null)} style={{
                  ...
                  [uiDir(chatLang) === "rtl" ? "left" : "right"]: "10px",
                  background: "none", border: "none", cursor: "pointer",
