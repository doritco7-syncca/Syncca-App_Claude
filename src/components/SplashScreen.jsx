...
// הקשת שלמה וסטטית. overlay של הרגל השמאלית מונפש מעל — אין קרעים.
...
export default function SplashScreen({ onDone }) {
  ...
  // הרגל השמאלית — path שמכסה בדיוק את הרגל מהקשת
  const LEFT_LEG = `M 111.28 85.30
    ...
    C 164.21 15.83 170.56 13.41 177.98 13.80
    L 111.28 85.30 Z`;

  // הקשת האדומה השלמה — סטטית
  ...
  const size = 180;
  const h = size * 289 / 357;

  return (
    <>
      <style>{`
        @keyframes left-leg-reach {
          0%   { transform: skewX(0deg)   scaleX(1);    }
          12%  { transform: skewX(-4deg)  scaleX(0.97); }
          42%  { transform: skewX(-24deg) scaleX(1.20); }
          62%  { transform: skewX(6deg)   scaleX(0.96); }
          78%  { transform: skewX(-1deg)  scaleX(1.01); }
          100% { transform: skewX(0deg)   scaleX(1);    }
        }
        .left-leg-overlay {
          transform-box: fill-box;
          transform-origin: 50% 0%;
          animation: left-leg-reach 2.6s cubic-bezier(0.68, -0.55, 0.27, 1.55) 0.7s 1 forwards;
        }
      `}</style>
      ...
      <div style={{
        ...
        <svg width={size} height={h} viewBox="0 0 357 289" fill="none"
             style={{ overflow: "visible" }}>
          {/* שכבה 1: קשת אדומה שלמה — סטטית, אין קרעים */}
          <path fill="#C62828" d={RED_FULL} />
          {/* שכבה 2: קשת אפורה — סטטית */}
          <path fill="#757575" d={GRAY} />
          {/* שכבה 3: overlay הרגל השמאלית — מונפש מעל, אותו צבע */}
          <path className="left-leg-overlay" fill="#C62828" d={LEFT_LEG} />
        </svg>
