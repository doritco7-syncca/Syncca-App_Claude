    ...
    <>
      <style>{`
        @keyframes logo-lean {
          ...
          15%  { transform: rotate(-2deg) skewX(-2deg); }
          /* שיא ההטיה — הלוגו כולו נשען שמאלה */
          40%  { transform: rotate(-8deg) skewX(-6deg); }
          /* snap back אלסטי */
          58%  { transform: rotate(3deg)  skewX(2deg);  }
          72%  { transform: rotate(-1deg) skewX(-1deg); }
          85%  { transform: rotate(1deg)  skewX(0.5deg);}
          100% { transform: rotate(0deg)  skewX(0deg);  }
        }
        .logo-anim {
          display: block;
          transform-origin: center bottom;
          animation: logo-lean 2.4s cubic-bezier(0.68, -0.55, 0.27, 1.55) 0.5s 1 forwards;
        }
      `}</style>
      ...
      <div style={{
        ...
        opacity: !visible ? 0 : fading ? 0 : 1,
        transition: `opacity ${fading ? "0.8s" : "0.5s"} ease`,
        pointerEvents: fading ? "none" : "auto",
      }}>

        <svg className="logo-anim"
             width="180" height={180 * 289/357}
             viewBox="0 0 357 289" fill="none">
          <path fill="#C62828" d={RED_FULL} />
